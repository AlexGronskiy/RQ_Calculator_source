...
def _format_bonus_line(template: str, value: int | float) -> str:
    ...
    # +Value, если шаблон сам не содержит знак перед {0}
...
def load_elixir_entries_from_db(conn) -> List[Dict[str, Any]]:
    """
    Возвращает entries для меню:
    [
        {
            "elixir": {"Id":..., "Name":..., "IsLegacy":..., "Image_Id":...},
            "bonuses": [...],
            "icon_pm": QPixmap(...)
        },
        ...
    ...
    try:
        elixir_rows = conn.execute(
            """
            SELECT Id, Name, IsLegacy, Image_Id
            FROM Elixir
            WHERE COALESCE(IsLegacy, 0) = 0
    ...
    elixirs: List[Dict[str, Any]] = []
    elixir_ids: List[int] = []
    icon_cache: Dict[int, QPixmap] = {}

    for r in elixir_rows or []:
        try:
            if hasattr(r, "keys"):
                eid = _to_int(r["Id"], 0)
                name = _to_str(r["Name"])
                is_legacy = _to_int(r["IsLegacy"], 0)
                image_id = _to_int(r["Image_Id"], 0)
            else:
                eid = _to_int(r[0], 0)
                name = _to_str(r[1])
                is_legacy = _to_int(r[2], 0)
                image_id = _to_int(r[3], 0)
        except Exception:
            continue
        ...
        elixir_ids.append(int(eid))

    if not elixir_ids:
        return elixirs

    ph = ",".join(["?"] * len(elixir_ids))

    try:
        bonus_rows = conn.execute(
            f"""
            SELECT Elixir_Id, Type_Id, Value, OrderIndex
            FROM ElixirBonus
            WHERE Elixir_Id IN ({ph})
            ORDER BY Elixir_Id, OrderIndex
            """,
            tuple(int(x) for x in elixir_ids),
        ).fetchall()
    except Exception:
        bonus_rows = []
    ...
    for r in bonus_rows or []:
        try:
            if hasattr(r, "keys"):
                elixir_id = _to_int(r["Elixir_Id"], 0)
                type_id = _to_int(r["Type_Id"], 0)
                value = _to_int(r["Value"], 0)
            else:
                elixir_id = _to_int(r[0], 0)
                type_id = _to_int(r[1], 0)
                value = _to_int(r[2], 0)
        except Exception:
            continue

        if elixir_id <= 0 or type_id <= 0:
            continue

        raw_bonus.append((int(elixir_id), int(type_id), int(value)))
    ...
    for elixir_id, type_id, value in raw_bonus:
        line = _format_bonus_line(templates.get(int(type_id), ""), value)
        bonus_map.setdefault(int(elixir_id), []).append(line)

    for e in elixirs:
        try:
            eid = int((e.get("elixir") or {}).get("Id") or 0)
        except Exception:
            eid = 0
        e["bonuses"] = list(bonus_map.get(eid, []))

    return elixirs


@dataclass
class ElixirChooseConfig:
    bg_path: str = "resources/elixir_menu/elixir_choose.png"
    ...
    block_bg_path: str = "resources/elixir_menu/elixir_block.png"
    ...
    # теперь бонусы стоят на месте колонки уровня / ближе к центру
    bonuses_rect: Tuple[int, int, int, int] = (240, 4, 165, 77)

    # вместо уровня справа индикатор выбранного эликсира
...
class _ElixirBlock(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self.elixir: Optional[Dict[str, Any]] = None
    ...
    def set_payload(self, elixir: Dict[str, Any], bonus_lines: List[str], icon_pm: Optional[QPixmap]) -> None:
        self.elixir = dict(elixir) if isinstance(elixir, dict) else None
    ...
    def mousePressEvent(self, ev) -> None:
        if ev.button() == Qt.LeftButton:
            self._pressed = bool(isinstance(self.elixir, dict) and self.elixir)
            ev.accept()
            return
    ...
    def mouseReleaseEvent(self, ev) -> None:
        ...
        if over and isinstance(self.elixir, dict) and self.elixir:
...
class ChooseElixirMenu(QWidget):
    picked = Signal(dict)
    closed = Signal()

    def __init__(self, parent: QWidget, *, config: Optional[ElixirChooseConfig] = None):
        super().__init__(parent, Qt.Popup | Qt.FramelessWindowHint)
        self.setAttribute(Qt.WA_TranslucentBackground, True)
        self.setAttribute(Qt.WA_NoMouseReplay, True)

        self.cfg = config or ElixirChooseConfig()
        ...
        self._blocks: List[_ElixirBlock] = []
        self._index_text: Dict[_ElixirBlock, str] = {}
        self._last_hover: Optional[_ElixirBlock] = None
        self._selected_elixir_id: int = 0
        ...
        self.search_edit.setPlaceholderText("Поиск эликсира (название / бонусы)")
    ...
    def _apply_window_mask_from_bg(self) -> None:
        """
        Обрезаем popup по альфе фоновой PNG,
        чтобы дочерние виджеты не торчали в прозрачных зонах картинки.
        """
        try:
            pm = self._bg_pm
        except Exception:
    ...
    def _set_close_pixmap(self, pm: Optional[QPixmap]) -> None:
        """
        Аккуратно кладём картинку крестика на полностью прозрачный холст
        того же размера, чтобы альфа PNG не терялась.
        """
        try:
            self._close.clear()
        except Exception:
    ...
    def selected_elixir_id(self) -> int:
        return int(self._selected_elixir_id or 0)

    def set_selected_elixir_id(self, elixir_id: int) -> None:
        self._selected_elixir_id = int(elixir_id or 0)
        for blk in self._blocks:
            try:
                eid = int((blk.elixir or {}).get("Id") or 0)
            except Exception:
                eid = 0
            blk.set_selected(eid > 0 and eid == self._selected_elixir_id)
    ...
    def open_for(
        ...
        *,
        ...
        entries = load_elixir_entries_from_db(conn)
        self.set_entries(entries)
        self.set_selected_elixir_id(selected_elixir_id)
    ...
    def _make_index_text(self, elixir: Dict[str, Any], bonus_lines: List[str]) -> str:
        name = str(elixir.get("Name") or "")
    ...
    def _build_blocks(self, entries: List[Dict[str, Any]]) -> None:
        ...
        for e in entries or []:
            ...
            blk = _ElixirBlock(self._cont, cfg=self.cfg, bg_pm=self._block_pm, size=bsz)
            blk.set_payload(elixir, lines, icon_pm)
            ...
            self._index_text[blk] = self._make_index_text(elixir, lines)

        self._vbox.addStretch(1)
        self.set_selected_elixir_id(self._selected_elixir_id)

    def _on_block_clicked(self, blk: _ElixirBlock) -> None:
        if not isinstance(getattr(blk, "elixir", None), dict) or not blk.elixir:
            return

        try:
            picked_id = int(blk.elixir.get("Id") or 0)
        except Exception:
            picked_id = 0
        ...
        self.set_selected_elixir_id(picked_id)

        cb = self._on_pick
        if callable(cb):
            try:
                cb(dict(blk.elixir), list(getattr(blk, "bonus_lines", []) or []))
            except Exception:
                pass

        try:
            self.picked.emit(dict(blk.elixir))
        except Exception:
            pass

    def _on_block_hover(self, blk: _ElixirBlock) -> None:
        self._last_hover = blk
        cb = self._on_hover_enter
        if callable(cb) and isinstance(getattr(blk, "elixir", None), dict) and blk.elixir:
            try:
                cb(blk, dict(blk.elixir), list(getattr(blk, "bonus_lines", []) or []))
            except Exception:
                pass

    def _on_block_unhover(self, blk: _ElixirBlock) -> None:
